...
- 二次确认（`confirm`）→ `DELETE` 目录，非 2xx 报"删除目录失败 HTTP n（只有空目录能删）"。
  > ⚠️ **"只有空目录能删"是应用侧的约束，不是服务器的。** 实测该 NAS 的 WebDAV 对**非空**集合发 `DELETE` 会**递归删掉整棵子树**并返回 `204`（用"一个含子目录 + 两个文件的目录"验证过，无回收站）。所以上面的删前 `PROPFIND` 复核是**唯一**的防线，任何"让服务器兜底"的改动都不能做。
